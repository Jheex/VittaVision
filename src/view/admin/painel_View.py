import streamlit as st
from model.oracle_connection import OracleDatabase


class AdminPainelView:
    """View do painel administrativo com cards inteiros clicáveis e design profissional."""

    def render(self):
        db = OracleDatabase()

        # Inicializa a aba ativa no session_state se não existir
        if "admin_aba_ativa" not in st.session_state:
            st.session_state.admin_aba_ativa = "👥 Usuários"

        # CSS Avançado para transformar botões em cards bonitos com efeito hover
        st.markdown(
            """
            <style>
                /* Estilização geral dos botões para parecerem cards modernos */
                div.stButton > button {
                    background: linear-gradient(145deg, rgba(18, 24, 38, 0.95) 0%, rgba(26, 16, 47, 0.95) 100%);
                    border: 1px solid rgba(168, 85, 247, 0.25);
                    border-radius: 16px;
                    color: #ffffff;
                    padding: 30px 20px;
                    text-align: left;
                    width: 100%;
                    box-shadow: 0 8px 32px rgba(0, 0, 0, 0.3);
                    transition: all 0.3s ease;
                }
                
                /* Efeito ao passar o mouse por cima do card */
                div.stButton > button:hover {
                    background: linear-gradient(145deg, rgba(28, 36, 58, 1) 0%, rgba(40, 24, 71, 1) 100%);
                    border-color: rgba(168, 85, 247, 0.8);
                    box-shadow: 0 12px 40px rgba(168, 85, 247, 0.25);
                    transform: translateY(-3px);
                }

                /* Ajuste de tipografia interna do botão */
                div.stButton > button p {
                    font-size: 18px !important;
                    font-weight: 600 !important;
                    color: #ffffff !important;
                    margin: 0px !important;
                }
            </style>
            """,
            unsafe_allow_html=True,
        )

        # Cabeçalho com informações da sessão
        st.info(f"Sessão ativa como **{st.session_state.get('admin_perfil', 'Administrador')}**.")
        st.write("")
        st.markdown("### 📊 Menu Principal")

        # ---------------------------------------------------------
        # 3 CARDS CLICÁVEIS EM COLUNAS
        # ---------------------------------------------------------
        c1, c2, c3 = st.columns(3)

        with c1:
            if st.button("👥 Usuários\n\nGerenciar acessos e cadastros do sistema.", key="card_usuarios", use_container_width=True):
                st.session_state.admin_aba_ativa = "👥 Usuários"
                st.rerun()

        with c2:
            if st.button("🗄️ Banco de Dados\n\nConsultas e status da conexão Oracle.", key="card_banco", use_container_width=True):
                st.session_state.admin_aba_ativa = "🗄️ Banco de Dados"
                st.rerun()

        with c3:
            if st.button("🚪 Sair\n\nEncerrar a sessão de administrador.", key="card_sair", use_container_width=True):
                st.session_state.admin_logado = False
                st.session_state.admin_perfil = ""
                st.session_state.admin_aba_ativa = "👥 Usuários"
                st.rerun()

        st.divider()

        # ---------------------------------------------------------
        # CONTEÚDO DOS MÓDULOS SELECIONADOS
        # ---------------------------------------------------------
        modulo_atual = st.session_state.admin_aba_ativa

        if modulo_atual == "👥 Usuários":
            st.subheader("👥 Módulo de Usuários")
            st.write("Gerenciamento rápido de cadastros na tabela `ALFA_USUARIO`.")
            try:
                conn = db._conectar()
                import pandas as pd
                df_usuarios = pd.read_sql("SELECT id_usuario, nm_login, ds_email, fl_ativo FROM ALFA_USUARIO", con=conn)
                conn.close()
                st.dataframe(df_usuarios, use_container_width=True)
            except Exception as e:
                st.error(f"Erro ao carregar usuários: {e}")

        elif modulo_atual == "🗄️ Banco de Dados":
            st.subheader("🗄️ Módulo de Banco de Dados")
            st.write("Status da conexão com o Oracle Autonomous Database ativo.")
            if st.button("Testar Conexão com Oracle"):
                try:
                    conn = db._conectar()
                    conn.close()
                    st.success("Conexão estabelecida com sucesso!")
                except Exception as e:
                    st.error(f"Falha na conexão: {e}")