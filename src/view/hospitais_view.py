...
class HospitaisView:
    ...
    def render(self, model=None):
        ...
        # 4. Mapa Geográfico Integrado (PyDeck)
    ...
    def _aplicar_estilos(self):
        st.markdown(
            ...
            /* Aplicando o gradiente diretamente no card de métrica nativo do Streamlit */
    ...
    def _render_graficos(self, df):
        ...
        with grafico1:
            ...
            if not df.empty and "TP_GESTAO" in df.columns:
                dicionario_gestao = {"M": "Municipal", "E": "Estadual", "D": "Privado"}
                gestao_series = df["TP_GESTAO"].map(dicionario_gestao).fillna(df["TP_GESTAO"])
                gestao_counts = gestao_series.value_counts()
    ...
    # =========================================================
    # MAPA INTERATIVO E DESTAQUES REGIONAIS (PYDECK)
    # =========================================================
    def _render_mapa_e_destaques(self, df):
        ...
        with map_col:
            ...
            # Define a câmera do mapa centralizada nos dados
            lat_central = df_mapa["lat"].mean() if not df_mapa.empty else -15.78
            lon_central = df_mapa["lon"].mean() if not df_mapa.empty else -47.92
            ...
            layer = pdk.Layer(
                ...
                get_radius="internacoes * 20",
                get_fill_color=[139, 92, 246, 180],
                pickable=True,
            )
            ...
            deck = pdk.Deck(
                ...
                layers=[layer],
                tooltip={"html": "<b>{nome}</b><br>Ocupação: {ocupacao}%<br>Volume/Leitos: {internacoes}"}
            )
